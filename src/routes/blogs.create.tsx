...
import { createBlogFn } from '@/server/blogs'
import { getBlogCategoriesFn } from '@/server/categories'

export const Route = createFileRoute('/blogs/create')({
  loader: async () => {
    await requireAuth()
    const categoriesResult = await getBlogCategoriesFn({ data: { page: 1, page_size: 100 } })
    return {
...
function CreateBlogPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      const result = await createBlogFn({
        data: {
          title: title.trim(),
    ...
    <AdminLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-[#2D3748]">Create Blog Post</h1>
        </div>
